...
Bitcoin addresses, \startingLTC{} Litecoin addresses, \startingDOGE{} Dogecoin
addresses, \startingETH{} Ethereum addresses and finally \startingXMR{} Monero
addresses. These results are depicted in~\autoref{fig:numberaccountcurrency}.
...
is still Bitcoin. The number of Monero wallets found is \startingXMR{}.
There are several possible explanations for this low finding. On the one
hand Monero has a smaller userbase than the other examined altcoins. On
the other hand it was born to preserve users' privacy, so people could be more
aware to advertise it.
...
because the former does not allow multiple-input transactions, so it is not
...
\clusteringNumberAllWallets{}, from which \clusteringNumberWalletsNotService{}
...
accounts to their Facebook and Linkedin contacts, respectively.
This two sets do not have intersection.
In particular the latter social network is the most interesting, because people
are more likely to share sensitive data on this platform because of its aim.
