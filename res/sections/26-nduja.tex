...
different source that we used, following the same interface
...
\item \addresschecker{} is a module that checks if a sequence of
characters recognized as an address is a real and used address. For certain
...
information
\item \graph{} is the module that creates graph with which is possible to
...
SQLite\footnote{\url{https://www.sqlite.org/}} database. 
Addresses are saved with the indication that if they are wallets retrieved from the websites we search,
...
The algorithm that \texttt{Nduja} follows could be explained with a sequence of
steps:
\subsubsection*{Data retrieving} First all the possible wallets are collected
and inserted into the database. So the research on Twitter and Github and using
Searchcode is performed. The addresses are stored connected to the account that
advertised them.
\subsubsection*{Account data retrieving} After all possible data related to
...
could be associate with the same person. \todo{Explain better}