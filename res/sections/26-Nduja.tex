...
and from multiple sources and they are stored in a database along with the
information of where thay were retrieved (i.e. reference to the account of the
...
\textit{Account} when we talk about a Twitter, Github
...
\autoref{fig:architecture} shows the architecture of Nduja and its algorithm.
This system was implemented using Python\footnote{In particular it could be ran using Python3+ and was tested only under Linux environment}
...
well-known services, also known as \emph{tagged
	addresses}\footnote{\url{https://blockchain.info/tags}}.
...
contained a really large set of addresses. By doing this we kept only about 45\% of the original addresses.
