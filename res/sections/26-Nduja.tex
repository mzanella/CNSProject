...
characters recognized as an address is a valid address and whether it is
involved at least in one transaction or not. For certain
...
\todo{Add sentence about the storage of these addresses to avoid explosion
	of addresses??? }
...
in the first step: this is done using related APIs
(e.g. Twitter API, Github Search API, Bitbucket API, etc.) but also searching for emails and websites in the same file
of the addresses.\todo{This is true for github but also for twitter???}
...
\todo{Explain that we used it only for Litecoin and Dogecoin, because
Ethereum does not have multiinput transactions}
